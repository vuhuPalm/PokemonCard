  ...
  selectQuery: function(tableName, options, model) {
    ...
    // Else do it the reguar way
      ...
      if (options.groupedLimit) {
        ...
        var distinct = '';
        if('distinctpg' in options) {
            if('distinct' in options) {
                distinct = options.distinct;
            }
        }
        mainQueryItems.push('SELECT '+ distinct + ' ' + mainAttributes.join(', ')+' FROM ('+
        ...
        mainQueryItems.push('SELECT ' + distinct + ' ' + mainAttributes.join(', ') + ' FROM ' + table);
      }
      if (mainTableAs) {
